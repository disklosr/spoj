...
namespace Katas.Solutions.CodeFights
{
    public class CheapMarquee
    {
        int Solve(string m, int w)
        {
            ...
            m = m.PadRight(Math.Max(m.Length, w)).ToUpper().Select(_ => _ - ' ').Where(c => c < 59 && u[c] != "0")
                .Aggregate("", (x, c) => x + (u[c] + "0"));

            return Enumerable.Range(0, m.Length - w - 1).Select(i => m.Skip(i).Take(w).Select(c => c - '0').Sum())
                .Max();
        }
